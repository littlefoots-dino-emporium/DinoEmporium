...
namespace DinoEmporium.Data
{
    public class PaymentInformationRepository
    {
        ...
        public PaymentInformation AddPaymentInformation(PaymentType paymentType, int accountNumber, string nameOnCard, DateTime expirationDate, int customerId, string address)
        {
            using (var db = new SqlConnection(ConnectionString))
            {
                var newPaymentInformation = db.QueryFirstOrDefault<PaymentInformation>(@"
                    Insert into paymentinformation (paymentType,accountNumber, nameOnCard, expirationDate, customerId, address)
                    Output inserted.*
                    Values(@paymentType,@accountNumber,@nameOnCard, @expirationDate, @customerId, @address)",
                    new { paymentType, accountNumber, nameOnCard, expirationDate, customerId, address});
